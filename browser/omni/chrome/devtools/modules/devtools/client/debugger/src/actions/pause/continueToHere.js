...
function continueToHere(cx, location) {
  return async function ({
    ...
    const pauseLocation = column && position ? position.location : location; // If we're replaying and the user selects a line above the currently
    ...
    const action = (0, _selectors.getCanRewind)(getState()) && line < debugLine ? _commands.rewind : _commands.resume; // Set a hidden breakpoint if we do not already have a breakpoint
