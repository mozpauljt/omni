...
function editorMenuItems({
  ...
  items.push(jumpToMappedLocationItem(cx, selectedSource, location, hasPrettySource, editorActions), continueToHereItem(cx, location, isPaused, editorActions), { type: "separator" }, ...(content && (0, _asyncValue.isFulfilled)(content) ? [copyToClipboardItem(content.value, editorActions)] : []), ...(!selectedSource.isWasm ? [copySourceItem(selectedSource, selectionText, editorActions), copySourceUri2Item(selectedSource, editorActions)] : []), ...(content && (0, _asyncValue.isFulfilled)(content) ? [downloadFileItem(selectedSource, content.value, editorActions)] : []), { type: "separator" }, showSourceMenuItem(cx, selectedSource, editorActions), blackBoxMenuItem(cx, selectedSource, editorActions));
