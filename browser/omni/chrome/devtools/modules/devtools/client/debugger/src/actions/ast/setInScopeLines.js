...
async function getInScopeLines(cx, location, { dispatch, getState, parser }) {
  ...
  const sourceNumLines = !content || !(0, _asyncValue.isFulfilled)(content) ? 0 : (0, _source.getSourceLineCount)(content.value);
...
function setInScopeLines(cx) {
  return async thunkArgs => {
    ...
    const { content } = (0, _selectors.getSourceWithContent)(getState(), location.sourceId);
